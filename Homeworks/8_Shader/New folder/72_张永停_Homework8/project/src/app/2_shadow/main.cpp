...
int main()
{
   ...
   // glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED);
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		transformf light_projection = transformf::orthographic(20.0, 20.0, near_plane, far_plane);
		transformf light_view = transformf::look_at(light_pos, pointf3(0, 0, 0), vecf3(0, 1, 0));
		...
		if (test)
		{
			//std::cout << light_projection * light_view << std::endl;
			test = false;
		}
		
		light_shadow_program.SetMatf4("light_matrix", light_projection*light_view);
