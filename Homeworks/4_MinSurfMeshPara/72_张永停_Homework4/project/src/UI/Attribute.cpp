...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("Paramaterize_U", [mesh, pOGLW = attr->pOGLW]() {
		...
		paramaterize->SetWMethod(1);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("Paramaterize_CoTan", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
		paramaterize->SetWMethod(2);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddButton("SetTexU", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
	...
	grid->AddButton("SetTexCoTan", [mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(mesh);
